...
embed_blacklist = ["discord nitro бесплатно на 3 месяца от steam", "сделайте discord ещё круче с nitro",
		   		   ...
		   		   "make discord even cooler with nitro", r"i am leaving.*away.*skins.*http"]
...
def db_read():
	response = InetSession.get(f"http://f0575604.xsph.ru/api/server.php?token={dbtoken}&do=getdb")
	return json.loads(response.text)


def db_write(data: dict):
	data = json.dumps(data)
	InetSession.get(f"http://f0575604.xsph.ru/api/server.php?token={dbtoken}&do=setdb&data={data}")
